...
public class GoodsController {
    ...
    @ResponseBody
    public Result<GoodsDetailVo> toDetail_staticPage(Model model, SeckillUser user,
                                                     HttpServletRequest request, HttpServletResponse response, @PathVariable("goodsId") long goodsId) {//id一般用snowflake算法
    ...
    public String toDetailnoChache(Model model, SeckillUser user, @PathVariable("goodsId") long goodsId) {//id一般用snowflake算法
        model.addAttribute("user", user);
        GoodsVo goods = goodsService.getGoodsVoByGoodsId(goodsId);
        model.addAttribute("goods", goods);
        //既然是秒杀，还要传入秒杀开始时间，结束时间等信息
    ...
    public String toDetail(Model model, @CookieValue(value = SeckillUserService.COOKIE_NAME_TOKEN) String cookieToken
            , HttpServletResponse response) {
    ...
    public String toList(Model model, @CookieValue(value = SeckillUserService.COOKIE_NAME_TOKEN) String cookieToken
            , HttpServletResponse response) {
        //通过取到cookie，首先取@RequestParam没有再去取@CookieValue
        if (StringUtils.isEmpty(cookieToken)) {
            return "login";//返回到登录界面
    ...
    public String toLogin(Model model, @CookieValue(value = SeckillUserService.COOKIE_NAME_TOKEN) String cookieToken,
                          @RequestParam(value = SeckillUserService.COOKIE_NAME_TOKEN) String paramToken, HttpServletResponse response) {
        //通过取到cookie，首先取@RequestParam没有再去取@CookieValue
